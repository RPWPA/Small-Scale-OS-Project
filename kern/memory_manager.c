...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* ptr_page_table_start_h_v=(uint32*)kmalloc(PAGE_SIZE);
	...
	{
		//cprintf("kmalloc done correctly\n");
	}

	uint32 ptr_page_table_start_h_ph=kheap_physical_address((unsigned int)ptr_page_table_start_h_v);

	//cprintf("after kheap_physical_address\n");
	...
	memset((uint32*)ptr_page_table_start_h_v , 0,PAGE_SIZE); //OR :(Page_SIZE/4)
	//cprintf("after memset\n");

	ptr_page_directory[PDX(virtual_address)] =CONSTRUCT_ENTRY((uint32)ptr_page_table_start_h_ph ,PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	//cprintf("after construct_entry\n");


	//	b.	clear the TLB cache (using "tlbflush()")
	tlbflush();
	//cprintf("after tlbflush\n");

	//change this "return" according to your answer
	//cprintf("ptr_page_table_start_h_v @ %x = %d \n",ptr_page_table_start_h_v,*ptr_page_table_start_h_v);
